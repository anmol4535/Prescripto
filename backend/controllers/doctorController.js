...
const appointmentDoctor = async (req, res) => {
  try {
    const docId = req.doctor.id;  // <-- use req.doctor.id, NOT req.body
    const appointments = await appointmentModel.find({ docId });

    res.json({ success: true, appointments });
  } catch (error) {
    console.error(error);  // Always log the real error
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export { changeAvalability,doctorProfile ,updateDoctorProfile,  appointmentDoctor, appointmentComplete, appointmentCancel, doctorList, doctorDashboard,clearDoctorSlots, loginDoctors };